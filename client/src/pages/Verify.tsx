...
export default function Verify() {
  ...
  const verify = trpc.auth.verifyEmail.useMutation({
    onSuccess: () => {
      setMessage("Email verified. Your learner record is now active.");
      window.setTimeout(() => navigate("/learn"), 700);
    },
  });
  const resend = trpc.auth.requestVerification.useMutation({
    onSuccess: result => setMessage(result.developmentCode ? `Development verification code: ${result.developmentCode}` : "A fresh code has been requested."),
  });
  ...
  return (
    <main className="auth-page">
      <section className="auth-card" aria-labelledby="verify-title">
        ...
        <p className="auth-lede">Enter the six-digit code sent to your email. Verification keeps certificates and account recovery tied to the right learner.</p>
        <form onSubmit={submit} className="auth-form">
          ...
          <Button type="submit" disabled={verify.isPending} className="auth-submit">{verify.isPending ? "Checking code…" : "Verify email"}<ArrowRight size={16} /></Button>
        </form>
        <button className="auth-back" onClick={() => resend.mutate()} disabled={resend.isPending}>Request another code</button>
