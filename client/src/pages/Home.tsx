...
function SourceLogo({ slug, name }: { slug: string; name: string }) {
  return <div className="rail-logo-frame"><img className="rail-logo" src={`https://api.iconify.design/simple-icons/${slug}.svg?color=%23ffffff`} alt={`${name} logo`} loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.style.display = "none"; event.currentTarget.parentElement?.classList.add("rail-logo-fallback"); }} /><span>{name.split(" ").map((word) => word[0]).join("").slice(0, 3)}</span></div>;
}
