...
export default function Home() {
  ...
  return <Shell><main className="academy-home">
    <section className="academy-hero" data-motion="hero" aria-labelledby="hero-title">
      <div className="hero-coordinate" aria-hidden="true"><span>BV—01</span><i /></div>
      <div className="hero-copy">
        <p className="eyebrow"><span className="lime-dot" /> BLACKVAULT ACADEMY / FIELD MANUAL 01</p>
        <div className="hero-lockup"><span>BLACKVAULT</span><b>/ RAMPAGE</b><small>OPEN LEARNING SYSTEMS</small></div>
        <h1 id="hero-title">Understand what<br /><em>makes it run.</em></h1>
        <p className="hero-intro">Rampage is a guided way into the systems behind modern computing. Follow the signal from packet to process, model to machine, and leave with work you can explain.</p>
        <div className="hero-actions"><Link className="primary-cta" href="/course/ai-systems">Enter AI Systems <ArrowUpRight size={18} /></Link><Link className="text-cta" href="/courses">View the course map <ArrowUpRight size={16} /></Link></div>
        <div className="hero-proof" aria-label="Rampage learning format">
          <div><strong>{spotlightCourse.phases.length}</strong><span>PHASES<br />IN FOCUS</span></div>
          <div><strong>{spotlightCourse.phases.reduce((sum, phase) => sum + phase.lessons.length, 0)}+</strong><span>LESSONS<br />TO TRACE</span></div>
          <div><strong>OPEN</strong><span>ORIGINAL<br />SOURCES</span></div>
        </div>
      </div>
      <div className="hero-rail" aria-label="Course in focus">
        <div className="hero-rail-top"><span>CURATED / OPEN / SERIOUS</span><b>RAMPAGE—01</b></div>
        <div className="hero-focus-card"><div className="hero-focus-orbit"><span>01</span><i /></div><p>COURSE IN FOCUS</p><strong>AI Systems</strong><small>Build the inference stack.<br />Trace the request end to end.</small><Link href="/course/ai-systems">Open course map <ArrowUpRight size={14} /></Link></div>
        <div className="hero-rail-bottom"><span>02—∞</span><small>NEXT PATHS<br />IN BUILD</small></div>
      </div>
    </section>
    ...
    <section className="catalog" id="catalog" data-motion="section"><div className="catalog-head"><div><div className="section-index">03 <span>/</span> THE CATALOG</div><h2>Paths in motion.</h2></div><p>Each course is a guided argument: here is the idea, here is the source, here is the exercise, and here is the next question. New routes can join the system without making the old ones harder to use.</p></div><Stagger className="course-grid">{courses.map((course, index) => <StaggerItem key={course.id}><Link href={course.phases.length ? `/course/${course.id}` : "/resources"} className={`course-card ${course.status === "SPOTLIGHT" ? "featured" : ""}`}><div className="course-card-top"><span>{String(index + 1).padStart(2, "0")}</span><span className={course.status === "SPOTLIGHT" ? "status-live" : "status-soon"}>{course.status}</span></div><div className="course-card-badge">{courseBadge(course.status)}</div><h3>{course.title}</h3><p>{course.subtitle}</p><div className="course-card-details"><span>{course.level}</span><span>{course.phases.length ? `${course.phases.reduce((sum, phase) => sum + phase.lessons.length, 0)} lessons` : "Curriculum in build"}</span></div><div className="course-card-foot"><span>{course.time}</span><ArrowUpRight size={17} /></div></Link></StaggerItem>)}</Stagger></section>
