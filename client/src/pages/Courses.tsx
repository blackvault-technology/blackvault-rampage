...
export default function Courses() {
  ...
  return <Shell><main className="courses-page">
    ...
    {spotlight && <section className="spotlight-launch-panel" aria-labelledby="spotlight-launch-title"><div className="spotlight-launch-panel__signal"><span className="lime-dot" /> NEW / FLAGSHIP ROUTE</div><div><p className="eyebrow">SPOTLIGHT COURSE / {spotlight.title}</p><h2 id="spotlight-launch-title">Start with C. See what the machine is doing.</h2><p>{spotlight.description}</p><div className="spotlight-launch-panel__proof"><span>{lessonCount(spotlight)} lessons</span><span>{spotlight.phases.length} phases</span><span>Source-led labs</span><span>Absolute Beginner → Systems Capable</span></div></div><Link className="primary-cta" href={`/course/${spotlight.id}`}>Open Spotlight Course <ArrowUpRight size={17} /></Link></section>}
